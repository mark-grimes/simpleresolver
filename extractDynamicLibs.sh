...
# grep argument to exclude libraries built by the project. They're
# added in the Dockerfile instead so that they're not located in the
# build directory on the container.
VETOED_LIBS="Telehist"
...
# Remove any of the vetoed libs from the list
REQUIRED_LIBS=`echo "$REQUIRED_LIBS" | grep -v "$VETOED_LIBS"`
